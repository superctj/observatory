...
def chunk_neighbor_tables_template(
    tables, column_name, n, max_length, max_row=None, max_token_per_cell=None
):
    """
    ...
    for table_index, df in enumerate(tables):
        ...
        if column_name not in df.columns:
            print(
                f"Column '{column_name}' not found in table {table_index}. "
                "Skipping..."
            )
            continue
        ...
        while start_row < chunk.shape[0]:
            ...
            # Yield the chunk with its start and end row indices and other
...
def chunk_neighbor_tables(
    ...
    for table_index, df in enumerate(tables):
        ...
        while start_row < chunk.shape[0]:
            ...
            if max_row:
                approxiamte_optimal_rows = min(
                    max_row, approxiamte_optimal_rows
                )
            ...
            if max_token_per_cell:
                ...
                for col in sub_chunk.columns:
                    sub_chunk.loc[:, col] = sub_chunk.loc[:, col].apply(
                        lambda x: truncate_cell(
                            str(x), tokenizer, max_token_per_cell
                        )
                    )

            # Assuming table2colList and truncate_index are some external
            # functions
            optimal_rows = column_based_truncate(
                sub_chunk, tokenizer, max_length, model_name
            )

            # Ensure optimal_rows is at least 1 to prevent infinite loops
            if optimal_rows == 0:
                print("Too many column headers. Consider reducing `max_col`.")
                print("You may also consider reducing `max_token_per_cell`.")
    ...
    for table_index, df in enumerate(tables):
        ...
        while start_col < df.shape[1]:
            ...
            while start_row < chunk.shape[0]:
                ...
                if max_token_per_cell:
                    approxiamte_optimal_rows = max_length // (
                        chunk.shape[1] * max_token_per_cell
                    )
                ...
                end_row = min(
                    start_row + approxiamte_optimal_rows, chunk.shape[0]
                )
                sub_chunk = chunk.iloc[start_row:end_row, :]

                if max_token_per_cell:
                    for col in sub_chunk.columns:
                        sub_chunk[col] = sub_chunk[col].apply(
                            lambda x: truncate_cell(
                                str(x), tokenizer, max_token_per_cell
                            )
                        )
                optimal_rows = column_based_truncate(
                    df, tokenizer, max_length, model_name
                )  # Assuming max_rows is some external function

                # Ensure optimal_rows is at least 1 to prevent infinite loops
                if optimal_rows == 0:
                    print(
                        "Too many column headers. Consider reducing `max_col`."
                    )
                    print(
                        "You may also consider reducing `max_token_per_cell`."
                    )
                    start_row = start_row + 1
                    continue
...
class TableColumnDataset(Dataset):
    def __init__(
        self, chunk_generator, tokenizer, max_length, padding_token, model_name
    ):
    ...
    def __getitem__(self, idx):
        ...
        if self.model_name.startswith("google/tapas"):
            ...
            attention_mask = [
                1 if token != self.padding_token else 0
