from observatory.common_util.column_based_truncate import max_rows, table2colList, max_rows
import pandas as pd
    ...
    for table_index, table in enumerate(tables):
        ...
        while start_col < df.shape[1]:
            ...
            while start_row < chunk.shape[0]:
                ...
                chunked_tables[table_index][(start_col, end_col)][(start_row, start_row+optimal_rows)] = truncated_chunk
...
class TableDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.model_name.startswith("google/tapas"):
            ...
            inputs = self.tokenizer(table=table, padding="max_length", return_tensors="pt", truncation=True)
            return {'inputs': inputs, 'position': table_position, 'index': table_index}
        else:
            cols = table2colList(table)
            processed_tokens, cls_positions = process_table(self.tokenizer, cols, self.max_length, self.model_name)
            input_ids = self.tokenizer.convert_tokens_to_ids(processed_tokens[0])
            attention_mask = [1 if token != self.padding_token else 0 for token in processed_tokens[0]]
            return {
...
def get_dataloader(tables, max_col, max_row, batch_size, tokenizer, max_length, model_name):
    chunked_tables_dict = chunk_tables(tables, max_col, max_row, tokenizer, max_length, model_name)
    chunked_tables = []
    for table_index, column_chunks in chunked_tables_dict.items():
        for position, chunk in column_chunks.items():
            chunked_tables.append({'table': chunk, 'position': position, 'index': table_index})
