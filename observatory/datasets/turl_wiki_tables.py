...
def prepare_data_for_turl(
    ...
    """
    Build a dictionary that maps entity ID in TURL to entity metadata from each
    table that contains the entity.

    More specifically, each entry in entity metadata is a list of inputs
    specifically for the TURL model.
    """
...
def prepare_data_for_lm(data_dir, entity_id_map) -> dict[int, List[List[str]]]:
    ...
    More specifically, each entry in entity metadata is a list of two strings:
    entity text and a concatenation of table metadata which includes caption,
    page title, section title and headers.
    """
...
class TurlWikiTableDataset(Dataset):
    ...
    def _preprocess_data(self):
        ...
        preprocessed_filename = os.path.join(
            self.data_dir, "preprocessed_hybrid", f"{self.split}.pickle"
        )
        ...
        with open(
            os.path.join(self.data_dir, f"{self.split}_tables.jsonl"), "r"
        ) as f:
            for line in tqdm(f):
                count += 1
                num_orig_tables += 1
                ...
                """
                If no entity cell is found under a header, this header needs to
                be removed

                Entity column indices also need to be remapped for creating
                masks later, e.g., [2, 4, 5] --> [0, 1, 2]
...
class TurlWikiTableCellDataset(Dataset):
    ...
    def _preprocess_data(self):
        ...
        line_exist = {}
        preprocessed_filename = os.path.join(
            self.data_dir, "preprocessed_hybrid", f"{self.split}_cells.pickle"
        )
        ...
        processed_data = list(
            tqdm(
                pool.imap(
                    partial(process_single_table_for_cells, config=self),
                    actual_tables,
                    chunksize=2000,
                ),
                total=len(actual_tables),
...
class entity_embedding_collate_fn:
    ...
    def __call__(self, raw_batch):
        (
            ...
            batch_entity_info,
            batch_input_tok,
            batch_input_tok_type,
            batch_input_tok_pos,
            batch_input_tok_mask,
            batch_input_tok_length,
            batch_input_ent,
            batch_input_ent_text,
            batch_input_ent_cell_length,
            batch_input_ent_type,
            batch_input_ent_mask,
            batch_input_ent_length,
            batch_column_header_mask,
            batch_column_entity_mask,
            batch_labels,
            batch_col_num,
        ) = zip(*raw_batch)
        ...
        batch_input_tok_mask_padded = np.zeros(
            [
                ...
                max_input_tok_length + max_input_ent_length,
            ],
            dtype=int,
        )
        ...
        batch_labels_padded = np.zeros(
            [batch_size, max_input_col_num, batch_labels[0].shape[-1]],
            dtype=int,
        )
        batch_labels_mask = np.zeros([batch_size, max_input_col_num], dtype=int)

        for i, (tok_l, ent_l, col_num) in enumerate(
            zip(batch_input_tok_length, batch_input_ent_length, batch_col_num)
        ):
            ...
            batch_input_tok_mask_padded[i, :tok_l, :tok_l] = (
                batch_input_tok_mask[i][0]
            )
            batch_input_tok_mask_padded[
                i, :tok_l, max_input_tok_length : max_input_tok_length + ent_l
            ] = batch_input_tok_mask[i][1]

            batch_input_ent_padded[i, :ent_l] = batch_input_ent[i]
            batch_input_ent_text_padded[
                i, :ent_l, : batch_input_ent_text[i].shape[-1]
            ] = batch_input_ent_text[i]
            batch_input_ent_text_length[i, :ent_l] = (
                batch_input_ent_cell_length[i]
            )
            batch_input_ent_type_padded[i, :ent_l] = batch_input_ent_type[i]
            batch_input_ent_mask_padded[i, :ent_l, :tok_l] = (
                batch_input_ent_mask[i][0]
            )
            batch_input_ent_mask_padded[
                i, :ent_l, max_input_tok_length : max_input_tok_length + ent_l
            ] = batch_input_ent_mask[i][1]
            batch_column_entity_mask_padded[i, :col_num, :ent_l] = (
                batch_column_entity_mask[i]
            )
            batch_column_entity_mask_padded[i, col_num:, 0] = 1
            batch_column_header_mask_padded[i, :col_num, :tok_l] = (
                batch_column_header_mask[i]
            )
        ...
        return (
            batch_table_id,
            batch_entity_info,
            batch_input_tok_padded,
            batch_input_tok_type_padded,
            batch_input_tok_pos_padded,
            batch_input_tok_mask_padded,
            batch_input_ent_text_padded,
            batch_input_ent_text_length,
            batch_input_ent_padded,
            batch_input_ent_type_padded,
            batch_input_ent_mask_padded,
            batch_column_entity_mask_padded,
            batch_column_header_mask_padded,
...
class EntityEmbeddingLoader(DataLoader):
    ...
    def __init__(
        ...
        self.is_train = is_train
        self.collate_fn = entity_embedding_collate_fn(
