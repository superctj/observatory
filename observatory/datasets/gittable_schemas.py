...
def get_gittable_schema_count(ACCESS_TOKEN, data_dir, jaccard_threshold):
    ...
    If two schemas have a high jaccard similarity, we treat them as the same
    schema. We do not double count the same schemas in the same repo.
    """

    r = requests.get(
        "https://zenodo.org/api/records/6517052",
    ...
    for f in os.listdir(data_dir):
        if f.endswith(".parquet"):
            ...
            try:
                ...
                print(f"Failed to read the table file {f} with error {e}")

            url = json.loads(
                table.schema.metadata["gittables".encode()].decode("utf-8")
            )["csv_url"]
...
def build_lm_input(headers, tokenizer):
    ...
    Also return a dictionary that maps attributes to their starting and ending
    positions so that we can extract the embedding for each attribute later.
    """
    ...
    for i in range(len(headers)):
        ...
        attr2pos[headers[i]] = (
            last_idx + 1,
            last_idx + tokenized_header_length + 1,
        )
...
def build_turl_input(headers, config):
    ...
    for i in range(len(tokenized_headers)):
        ...
        attr2pos[headers[i]] = (
            len(input_tok),
            len(input_tok) + tokenized_header_length,
        )
...
def build_tapas_input(headers):
    ...
    The attribute would directly be the query to TAPAS, so we do not need
    attr2pos here.
    """
...
if __name__ == "__main__":
    ACCESS_TOKEN = (
        "nBM6EEUv1uq0N8XBux3cQLuWEf1gjMyTLHyxsAaDWyn5Ow0AAgxPacYLZsHE"
    )
