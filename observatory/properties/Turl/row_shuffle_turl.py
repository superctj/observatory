...
if __name__ == "__main__":
    ...
    save_directory_results  = os.path.join('/nfs/turbo/coe-jag/zjsun', 'row_insig', args.save_directory, 'Turl' ,'results')
    save_directory_embeddings  = os.path.join('/nfs/turbo/coe-jag/zjsun', 'row_insig', args.save_directory, 'Turl' ,'embeddings')
            ...
            for batch in test_dataloader:
                ...
                with torch.no_grad():
                    ...
                    if all_shuffled_embeddings == []:
                        ...
                        # print(all_shuffled_embeddings.size())
                        if all_shuffled_embeddings[0].size() == col_embeddings[0].size():
                            all_shuffled_embeddings = torch.cat((all_shuffled_embeddings, col_embeddings), dim=0)
                        else:
                            print("\n\n\n This shouldn't occur too often!!!!\n\n\n")
