...
# def get_subsets(n, m, portion, original_entity_columns, original_entity_cells):
#     portion_size = int(n * portion)

#     # Generate all possible subsets
#     all_subsets = list(itertools.combinations(range(n), portion_size))

#     # Filter subsets to only include those with at least one entity cell under all entity columns
#     valid_subsets = []
#     for subset in all_subsets:
#         if all(any(original_entity_cells[i][col] for i in subset) for col in original_entity_columns):
#             valid_subsets.append(subset)

#     if len(valid_subsets) <= m:
#         # If the number of valid combinations is small, return all of them
#         return [list(subset) for subset in valid_subsets]
#     else:
#         # If the number of valid combinations is large, randomly select m of them
#         random.shuffle(valid_subsets)
#         return [list(subset) for subset in valid_subsets[:m]]    
def explore_combinations(n, subset, col_index, original_entity_columns, original_entity_cells):
    # Base case: if we've considered all columns, yield the subset
    if col_index == len(original_entity_columns):
        yield subset
    else:
        col = original_entity_columns[col_index]
        entity_rows = [i for i in range(n) if original_entity_cells[i][col] == 1]
        for row in entity_rows:
            # Skip this row if it's already in the subset and there are other entity cells in this column
            if any(original_entity_cells[i][col] for i in subset):
                yield from explore_combinations(n, subset, col_index + 1, original_entity_columns, original_entity_cells)
            if row not in subset:
                # Otherwise, add this row to the subset and move on to the next column
                new_subset = subset + [row]
                yield from explore_combinations(n, new_subset, col_index + 1, original_entity_columns, original_entity_cells)

# def get_subsets(n, m, portion, original_entity_columns, original_entity_cells):
#     portion_size = int(n * portion)
#     valid_subsets = []
#     original_entity_columns = [col for col in original_entity_columns if any(original_entity_cells[i][col] for i in range(n))]

#     # Generate all possible subsets
#     for subset in explore_combinations(n, [], 0, original_entity_columns, original_entity_cells):
#         # If the number of selected rows is less than the portion size, randomly select more rows
#         if len(subset) < portion_size:
#             remaining_rows = list(set(range(n)) - set(subset))
#             subset.extend(random.sample(remaining_rows, portion_size - len(subset)))
#         # If the number of selected rows is more than the portion size, abandon the subset
#         elif len(subset) > portion_size:
#             continue
#         # If the subset is of the correct size, add it to the list of valid subsets
#         if len(subset) == portion_size:
#             subset.sort()
...
def get_subsets(n, m, portion, original_entity_columns, original_entity_cells):
    ...
    # Generate all possible subsets
    combinations_generator = explore_combinations(n, [], 0, original_entity_columns, original_entity_cells)

    while len(valid_subsets) < m:
        try:
            subset = next(combinations_generator)
        except StopIteration:
            break  # Stop if there are no more combinations

        # If the number of selected rows is less than the portion size, add all possible combinations
        if len(subset) < portion_size:
            remaining_rows = list(set(range(n)) - set(subset))
            for remaining_subset in combinations(remaining_rows, portion_size - len(subset)):
                new_subset = subset + list(remaining_subset)
                new_subset.sort()
                if new_subset not in valid_subsets:
...
def shuffle_lists(lists, perms):
    ...
    # # If the original list is in the shuffled lists more than once, remove the extra occurrence
    # if shuffled_lists.count(lists) > 1:
    #     shuffled_lists.remove(lists)
    return shuffled_lists
class TurlWikiTableDataset(Dataset):
    def __init__(self, line, entity_vocab: dict, tokenizer, split: str, portion: float, max_columns: int = 10, max_cells: int = 100, max_input_tokens: int = 350, max_input_entities: int = 150, max_title_length: int = 50, max_header_length: int = 10, max_cell_length: int = 10, force_new: bool = False, num_change: int =1000):
    ...
    def _preprocess_data(self):
        ...
        print("=" * 50)
        # Get permutations
        perms = get_subsets(len(original_rows), self.num_change,  self.portion, actual_entity_columns, real_entity_cells)
