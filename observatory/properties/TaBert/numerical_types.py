...
if __name__ == "__main__":
    ...
    for _, row in data_loader.metadata.iterrows():
        ...
        try:
            col_itself.append((get_average_embedding(numerical_col.copy(), 0, n,  get_embedding), row["label"]))
            subj_col_as_context.append((get_average_embedding(two_col_table.copy(), 1, n,  get_embedding), row["label"]))
            neighbor_col_as_context.append((get_average_embedding(three_col_table.copy(), 1, n,  get_embedding), row["label"]))
            entire_table_as_context.append((get_average_embedding(table.copy(), numerical_col_idx, n,  get_embedding), row["label"]))
