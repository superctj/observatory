        ...
        for file_name in file_list:
            ...
            if file_name.endswith("results.pt"):
                ...
                for results in results_list:
                    ...
                    if (
                        "multiset_jaccard_similarity" in results
                        and "weighted_jaccard_coeefient" not in results
                    ):
                        old_value = results["multiset_jaccard_similarity"]

                        # update keys as required
                        results["weighted_jaccard_coeefient"] = old_value
                        results["multiset_jaccard_similarity"] = old_value / (
                            1 + old_value
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path", type=str, required=True, help="Path to the root directory"
    )
