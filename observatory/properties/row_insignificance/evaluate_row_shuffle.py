...
from row_shuffle import row_shuffle
...
def generate_row_shuffle_embeddings(tokenizer, model, device, max_length, padding_token, table, num_shuffles):
...
def analyze_embeddings(all_shuffled_embeddings, num_shuffles):
    ...
    for i in range(len(all_shuffled_embeddings[0])):
        ...
        for j in range(1, num_shuffles + 1):
...
def process_table_wrapper(table_index, truncated_table, args, model_name, model, tokenizer, device, max_length, padding_token):
    ...
    all_shuffled_embeddings = generate_row_shuffle_embeddings(tokenizer, model, device, max_length, padding_token, truncated_table, args.num_shuffles)
    avg_cosine_similarities, mcvs, table_avg_cosine_similarity, table_avg_mcv = analyze_embeddings(all_shuffled_embeddings, args.num_shuffles)
    results = {
