...
def load_and_process_data(get_embedding, save_directory, start, ifentity_info =False,  data_dir= "/home/zjsun/Turl", min_ent_count=2):
            ...
            for table_data in data:
                ...
                if  ifentity_info:
                    ...
                    try:
                        ...
                        print(e)
                        continue
                    torch.save(cell_embeddings, os.path.join(save_directory_cell,  f"line{count}_cell_embeddings.pt"))
                    entity_embeddings = {}
                    for index, entity in tmp_entities:
                        try:
                            i, j = index
                            entity_embeddings[(i,j)] = (cell_embeddings[i+1][j], entity[0])
                        except IndexError:
                            continue
                    torch.save(entity_embeddings, os.path.join(save_directory_entity,  f"line{count}_entity_embeddings.pt"))
...
if __name__ == "__main__":
    ...
    save_directory  = os.path.join('/nfs/turbo/coe-jag/zjsun', 'cell_embeddings', model_name)
