...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute average variances of L2 norms."
    )
    parser.add_argument(
        "--folders", nargs="+", required=True, help="List of folder paths"
    )
    parser.add_argument(
        "--labels", nargs="+", required=True, help="List of corresponding labels"
    )
    parser.add_argument("--output", default="results.txt", help="Output file name")
    parser.add_argument(
        "--min_length",
        type=int,
        default=1,
        help="Minimum list length to include in variance calculation",
    )

    args = parser.parse_args()

    assert len(args.folders) == len(
        args.labels
    ), "Number of folders and labels must be equal"

    with open(args.output, "w") as f:
        for folder, label in zip(args.folders, args.labels):
            print(f"Processing label: {label}")
            avg_variance_pairs, avg_variance_non_pairs = compute_avg_variance(
                folder, args.min_length
            )
            f.write(
