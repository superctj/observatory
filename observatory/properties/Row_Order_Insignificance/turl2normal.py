...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="This script processes a jsonl file and saves each table as a separate csv file in the given directory."
    )
    parser.add_argument(
        "-f",
        "--file_path",
        type=str,
        required=True,
        help="Path to the jsonl file to be processed. For example: -f dev_tables.jsonl",
    )
    parser.add_argument(
        "-d",
        "--directory",
        type=str,
        required=True,
        help="Directory where the csv files will be saved. For example: -d tables_csv",
    )
