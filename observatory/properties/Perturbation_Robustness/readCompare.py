...
def compare_directories(original_dir, modified_dir):
    ...
    for folder in original_folders:
        ...
        modified_folders = [
            ...
            if d.startswith(folder) and os.path.isdir(os.path.join(modified_dir, d))
        ]
        ...
        for mod_folder in modified_folders:
            ...
            original_files = [
                f
                for f in os.listdir(os.path.join(original_dir, folder))
                if f.endswith(".csv")
            ]
            modified_files = [
                f
                for f in os.listdir(os.path.join(modified_dir, mod_folder))
                if f.endswith(".csv")
            ]
            ...
            for file in original_files:
                ...
                if file in modified_files:
                    ...
                    modified_df = pd.read_csv(
                        os.path.join(modified_dir, mod_folder, file)
                    )

                    # Find the columns that changed from the original table in the modified table
                    changed_columns_index = [
                        i
                        for i, (col_orig, col_mod) in enumerate(
                            zip(original_df.columns, modified_df.columns)
                        )
                        if col_orig != col_mod
                    ]

                    # Add the result to the dictionary
                    filename_without_extension = os.path.splitext(file)[
                        0
                    ]  # Remove .csv from file
