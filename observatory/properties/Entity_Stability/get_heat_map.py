...
def load_embeddings_and_ids_from_directories(directory):
    ...
    for file in os.listdir(directory):
        if file.endswith(".pt"):
            ...
            data = torch.load(
                os.path.join(directory, file), map_location=torch.device("cpu")
            )
...
def get_top_k_similar(query_embedding, embeddings, K):
    ...
    embeddings_values = np.array(
        [value.detach().cpu().numpy() for _, value in embeddings_list]
    )

    similarities = cosine_similarity(
        [query_embedding.detach().cpu().numpy()], embeddings_values
    )[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--directories", type=str, nargs="+", help="List of directories"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        help="List of K values for top-K similar cells",
    )
    parser.add_argument(
        "--if_only_plot", type=str, default="False", help="Whether only do the plots"
    )
    parser.add_argument(
        "--if_double_entity",
        type=str,
        default="False",
        help="Whether fix the entity similarity",
    )
    ...
    if args.if_only_plot == "True":
        for K in K_values:
            with open(
                os.path.join(save_directory, f"overlaps_dict_K{K}.pkl"), "rb"
            ) as f:
                overlaps_dict = pickle.load(f)
            with open(
                os.path.join(save_directory, f"entity_similarity_dict_K{K}.pkl"), "rb"
            ) as f:
                entity_similarity_dict = pickle.load(f)
            if args.if_double_entity == "True":
                new_entity_similarity_dict = {
                    k: v * 2.0 for k, v in entity_similarity_dict.items()
                }
                entity_similarity_dict = new_entity_similarity_dict
                with open(
                    os.path.join(save_directory, f"entity_similarity_dict_K{K}.pkl"),
                    "wb",
                ) as f:
    ...
    for key in selected_keys:
        ...
        for embeddings, entity_ids in zip(all_embeddings, all_entity_ids):
            ...
            key_entity_ids_all_K.append(
                {K: [entity_ids[k] for k in similar_keys[-K:]] for K in K_values}
            )
        ...
        for K in K_values:
            ...
            for i in range(len(labels)):
                ...
                for j in range(i + 1, len(labels)):
                    overlap_count = len(
                        set(key_similarities_all_K[i][K])
                        & set(key_similarities_all_K[j][K])
                    )
                    overlaps_key[i, j] = overlap_count / K
                    overlaps_key[j, i] = overlap_count / K
                    entity_overlap = multiset_jaccard_similarity(
                        key_entity_ids_all_K[i][K], key_entity_ids_all_K[j][K]
                    )
    ...
    for K in K_values:
        ...
        with open(
            os.path.join(save_directory, f"entity_similarity_dict_K{K}.pkl"), "wb"
        ) as f:
            pickle.dump(entity_similarity_dict_all_K[K], f)

        position_overlaps = np.mean(list(overlaps_dict_all_K[K].values()), axis=0)
        entity_similarity = np.mean(
            list(entity_similarity_dict_all_K[K].values()), axis=0
        )
        # 4. Plot heatmaps
        plt.figure(figsize=(10, 10))
        sns.heatmap(
