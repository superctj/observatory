...
def plot_cosine_similarity_distribution(read_folders, labels, save_folder, picture_name):
    ...
    ax1.boxplot([value[0] for key, value in data.items()], labels=[key for key, value in data.items()])
    ...
    ax2.boxplot([value[1] for key, value in data.items()], labels=[key for key, value in data.items()])
    ax2.set_ylabel('mcv')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--read_folders', type=str, nargs='+', required=True, help='The folders to read the results from')
    parser.add_argument('--labels', type=str, nargs='+', required=True, help='The labels corresponding to each read folder')
    parser.add_argument('--save_folder', type=str, required=True, help='The folder to save the plot in')
    parser.add_argument('--picture_name', type=str, required=True, help='The name of the picture to save')

    args = parser.parse_args()

    plot_cosine_similarity_distribution(args.read_folders, args.labels, args.save_folder, args.picture_name)
