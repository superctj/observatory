...
def compute_covariance_matrix(embeddings):
    ...
    covariance_matrix = torch.matmul(zero_mean_embeddings.T, zero_mean_embeddings) / (
...
def compute_mcv(embeddings):
    ...
    mcv = torch.sqrt(
        torch.matmul(mean_vector.T, torch.matmul(covariance_matrix, mean_vector))
        / (torch.matmul(mean_vector.T, mean_vector) ** 2)
    )
