...
def main(args):
    ...
    for i, table in enumerate(normal_tables):
        ...
        processed_table = process_table(
            tokenizer, col_list, max_length, args.model_name
        )
        input_ids = tokenizer.convert_tokens_to_ids(processed_table[0])
        attention_mask = [
            1 if token != padding_token else 0 for token in processed_table[0]
        ]
        ...
        np.save(
            os.path.join(args.save_directory, f"table_{i + 1}_embeddings.npy"),
            embeddings,
        )
