...
import torch
import numpy as np
from observatory.observatory.models.huggingface_models import load_transformers_tokenizer, load_transformers_model
...
def process_table(tokenizer, cols, max_length):
    current_tokens = []
    cls_positions = []
    ...
    if len(current_tokens) < max_length:
        ...
        current_tokens += ['[PAD]'] * padding_length

    return current_tokens, cls_positions


def main(args):
    tokenizer = load_transformers_tokenizer(args.model_name)
    ...
    for i, table in enumerate(normal_tables):
        ...
        processed_table = process_table(tokenizer, col_list, args.max_length)
        input_ids = tokenizer.convert_tokens_to_ids(processed_table[0])
        attention_mask = [1 if token != '[PAD]' else 0 for token in processed_table[0]]
