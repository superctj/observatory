...
tapas_tokenizer = TapasTokenizer.from_pretrained("google/tapas-base", drop_rows_to_fit=True)
...
with open(os.path.join(data_dir, "test_tables.jsonl"), "r") as f:
    for table in tqdm(f):
        ...
        dup_id = set()
        table_dict = {'pgTitle': [pgTitle], 'sectionTitle': [secTitle], 'tableCaption': [caption], 'tableHeaders': [' '.join(headers)]}
...
for i in range(4, sample_size+4):
    ...
    with torch.no_grad():
        ...
        # for j in range(len(id2table[i])):
            if no_metadata:
                base_input = tapas_tokenizer(table=pd.DataFrame(), queries=[entity_vocab[i]['wiki_title']], truncation=True, return_tensors="pt")
                tok_outputs_meta, ent_outputs_meta, _ = model.table(None, None, None, None,
                                None, None, None,
                                torch.LongTensor([[i]]).to(device), None, None, None)
            else:
                base_input = tapas_tokenizer(table=id2data[i][j], queries=[entity_vocab[i]['wiki_title']], truncation=True, return_tensors="pt")
                tok_outputs_meta, ent_outputs_meta, _ = model.table(id2table[i][j][0], id2table[i][j][1], id2table[i][j][2], None,
                                None, None, None,
                                torch.LongTensor([[i]]).to(device), None, None, None)
...
if no_metadata:
    ...
    base_name = 'tapas_emb_all_metadata.pkl' if all_metadata else 'tapas_emb_one_metadata.pkl'
    turl_name = 'turl_emb_all_metadata.pkl' if all_metadata else 'turl_emb_one_metadata.pkl'
