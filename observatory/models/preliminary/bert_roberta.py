...
def get_entity_id_map(data_dir, min_ent_count, sample_size) -> dict:
    ...
    entity_id_map = {int(entity_vocab[x]["wiki_id"]): x for x in entity_vocab if x <= sample_size + 3 and x >= 4} # first four ids are reserved for special tokens, actual entity id starts at 4
...
def prepare_data_for_turl(data_dir, entity_vocab, entity_id_map, tokenizer, device):
    ...
    with open(os.path.join(data_dir, "test_tables.jsonl"), "r") as f:
        for table in tqdm(f):
            ...
            input_token, input_token_type, input_token_pos = build_metadata_input_for_turl(pgTitle, secTitle, caption, headers, dataset)
            dup_id = set()
...
def bert_embedding_inference(tokenizer, model, sample_size, id2metadata, all_metadata: bool, no_metadata: bool, device):
    count = 0
    bert_embeddings = {}
...
def test_lm():
    ...
    bert_embeddings = bert_embedding_inference(bert_tokenizer, bert_model, sample_size, id2metadata, all_metadata, no_metadata, device)
