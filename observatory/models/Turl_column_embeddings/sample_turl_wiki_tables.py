...
def build_metadata_input_for_turl(
    page_title: str, section_title: str, caption: str, headers: List[str], config
            ...
            for row in rows:
                for cell in row:
                    ...
                    id2metadata[id].append(
                        [
                            entity_text,
                            " ".join([caption, page_title, section_title] + headers),
                        ]
                    )
    return id2metadata
...
def process_single_table_for_columns(input_table, config):
    ...
    for e_i, (index, (entity_id, entity_text)) in enumerate(entities):
        tokenized_entity_text = config.tokenizer.encode(
            entity_text, max_length=config.max_cell_length, add_special_tokens=False
        )
    ...
    if pg_entity != -1:
        ...
        input_tok_mask[1] = np.concatenate(
            [np.zeros([len(input_tok), 1], dtype=int), input_tok_mask[1]], axis=1
        )
...
def explore_combinations(
    n, subset, col_index, original_entity_columns, original_entity_cells
):
    ...
    if col_index == len(original_entity_columns):
        ...
        for row in entity_rows:
            ...
            if any(original_entity_cells[i][col] for i in subset):
                yield from explore_combinations(
                    n,
                    subset,
                    col_index + 1,
                    original_entity_columns,
                    original_entity_cells,
                )
...
def get_subsets(n, m, portion, original_entity_columns, original_entity_cells):
    ...
    # Generate all possible subsets
    combinations_generator = explore_combinations(
        n, [], 0, original_entity_columns, original_entity_cells
    )
    ...
    while len(valid_subsets) < m:
        ...
        if len(subset) < portion_size:
            remaining_rows = list(set(range(n)) - set(subset))
            for remaining_subset in combinations(
                remaining_rows, portion_size - len(subset)
            ):
...
class TurlWikiTableDataset(Dataset):
    ...
    def _preprocess_data(self):
                    ...
                    if real_entity_cells[i][j] == 1:
                        try:
                            entity_id = self.entity_id_map[
                                original_rows[i][j]["surfaceLinks"][0]["target"]["id"]
                            ]
        ...
        real_entity_columns = [
            col
            for col in original_entity_columns
            if any(real_entity_cells[i][col] for i in range(len(real_entity_cells)))
        ]
