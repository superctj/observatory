...
if __name__ == "__main__":
    ...
    with open(os.path.join(data_dir, "test_tables.jsonl"), "r") as f:
        ...
        for table_index in range(args.start_line, len(lines)):
            ...
            with open("col_demo_message.txt", "w") as message:
                message.write(str("original_entity_column:") + "\n")
                message.write(str(original_entity_column) + "\n")
                for j in range(1, len(perms)):
                    ...
                    message.write(str(entity_column) + "\n")

                    new_perm = [
                        original_entity_column.index(index) for index in entity_column
                    ]
